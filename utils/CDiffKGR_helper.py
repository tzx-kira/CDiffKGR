...
def _build_generated_kg(pmi_dict, n_items, kg_dict, item_embeds=[],
                       pmi_threshold=0.5, cos_threshold=0.9,
                       use_tdm=False, tdm_model=None, diffusion_model=None,
                       entity_embeds=None, relation_embeds=None, device=None, args=None):
    """
    生成知识图谱三元组
    注意：此函数现在被保留，但主要功能已在CDiffKGR_main.py中实现
    """
    if use_tdm and tdm_model is not None:
        # 使用TDM模型生成三元组
        log_to_file("使用TDM模型生成三元组...")
        # 这里返回空数组，实际在CDiffKGR_main.py中处理
        return np.array([])
    else:
        # 使用原始方法生成三元组
        ...
        for k, v in new_kg_dict.items():
            for r, t in v:
                generated_kg.append([k, r, t])
        generated_kg = np.array(generated_kg)
        generated_kg = np.unique(generated_kg, axis=0)
        print("generated KG size: ", generated_kg.shape[0])
        return generated_kg
